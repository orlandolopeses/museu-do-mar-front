...
async function main() {
  ...
  if (reusedServer) {
    console.log(`Runtime já ativo em ${baseUrl}; reutilizando para o smoke do bolsista.`);
  } else {
  ...
  try {
    const exitCode = await runChild("node scripts/validate-bolsista-scope.mjs", {
      NEXTAUTH_URL: baseUrl,
      SCOPE_SMOKE_BASE_URL: baseUrl,
    });

    if (exitCode !== 0) {
      throw new Error(`Smoke do bolsista falhou com saída ${exitCode}.`);
    }

    console.log("Smoke gerenciado do bolsista concluído com sucesso.");
  } finally {
    if (serverChild) {
      console.log("Encerrando runtime temporário do smoke do bolsista...");
      await terminateChild(serverChild);
    }
  }
}
...
main().catch((error) => {
  console.error("Falha ao executar smoke gerenciado do bolsista:", error.message);
  process.exit(1);
});