...
async function main() {
  ...
  if (reusedServer) {
    console.log(`Runtime já ativo em ${baseUrl}; reutilizando para o smoke da equipe de produção.`);
  } else {
  ...
  try {
    const exitCode = await runChild("node scripts/validate-producao-scope.mjs", {
      NEXTAUTH_URL: baseUrl,
      SCOPE_SMOKE_BASE_URL: baseUrl,
    });

    if (exitCode !== 0) {
      throw new Error(`Smoke da equipe de produção falhou com saída ${exitCode}.`);
    }

    console.log("Smoke gerenciado da equipe de produção concluído com sucesso.");
  } finally {
    if (serverChild) {
      console.log("Encerrando runtime temporário do smoke da equipe de produção...");
      await terminateChild(serverChild);
    }
  }
}
...
main().catch((error) => {
  console.error("Falha ao executar smoke gerenciado da equipe de produção:", error.message);
  process.exit(1);
});