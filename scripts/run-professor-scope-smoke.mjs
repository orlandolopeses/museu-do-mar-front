...
async function main() {
  ...
  if (reusedServer) {
    console.log(`Runtime já ativo em ${baseUrl}; reutilizando para o smoke.`);
  } else {
  ...
  try {
    const exitCode = await runChild("node scripts/validate-professor-scope.mjs", {
      NEXTAUTH_URL: baseUrl,
      SCOPE_SMOKE_BASE_URL: baseUrl,
    });

    if (exitCode !== 0) {
      throw new Error(`Smoke do professor falhou com saída ${exitCode}.`);
    }

    console.log("Smoke gerenciado do professor concluído com sucesso.");
  } finally {
    if (serverChild) {
      console.log("Encerrando runtime temporário do smoke...");
      await terminateChild(serverChild);
    }
  }
}
...
main().catch((error) => {
  console.error("Falha ao executar smoke gerenciado do professor:", error.message);
  process.exit(1);
});