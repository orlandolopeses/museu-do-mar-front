...
async function login(jar, email, password) {
  ...
  const body = new URLSearchParams({
    ...
    email,
    password,
    callbackUrl: `${baseUrl}/app/equipe-producao`,
    json: "true",
  });
  ...
  assert([200, 302, 303].includes(loginResponse.status), `Falha no callback de credenciais da produção: HTTP ${loginResponse.status}.`);

  const sessionResponse = await request(jar, "/api/auth/session");
  assert(sessionResponse.ok, `Falha ao ler sessão autenticada da produção: HTTP ${sessionResponse.status}.`);
  const session = await sessionResponse.json();
  assert(session?.user?.email === email, "Sessão autenticada não pertence ao usuário de produção criado para o smoke.");
}
...
async function createFixture(sql, suffix) {
  ...
  const roleId = await getRoleId(sql, "equipe_producao");
  const userId = randomUUID();
  const email = `equipe.producao.smoke.${suffix}@museudomar.local`;
  const password = `EquipeProducaoSmoke!${suffix}`;
  const passwordHash = await bcrypt.hash(password, 10);
  const ownEventId = randomUUID();
  const foreignInstitutionId = randomUUID();
  const foreignEventId = randomUUID();
  const ownEventTitle = `Frente operacional smoke ${suffix}`;
  const foreignEventTitle = `Frente externa smoke ${suffix}`;
  const foreignInstitutionName = `Instituição externa produção ${suffix}`;
  ...
  await sql`
    insert into user_instituicoes (
      ...
      ${userId},
      ${institutionId},
      ${"Equipe operacional de homologação"},
      true,
      now()
    )
  `;
  ...
  await sql`
    insert into eventos (
      ...
      ${ownEventId},
      ${ownEventTitle},
      ${"Evento de prova operacional da equipe de produção."},
      ${institutionId},
      ${"Base local de homologação"},
      ${new Date("2026-05-10T14:00:00-03:00")},
      ${new Date("2026-05-10T17:00:00-03:00")},
      ${"Operação"},
      ${null},
      ${null},
      true,
  ...
  await sql`
    insert into eventos (
      ...
      ${foreignEventId},
      ${foreignEventTitle},
      ${"Evento externo que não deve aparecer no recorte da produção."},
      ${foreignInstitutionId},
      ${"Território externo"},
      ${new Date("2026-05-11T14:00:00-03:00")},
      ${new Date("2026-05-11T16:00:00-03:00")},
      ${"Operação"},
      ${null},
      ${null},
      true,
...
async function fetchDashboard(jar, ownEventTitle, foreignEventTitle) {
  const dashboardResponse = await request(jar, "/app/equipe-producao");
  assert(dashboardResponse.ok, `Painel da equipe de produção retornou HTTP ${dashboardResponse.status}.`);
  const dashboardHtml = await dashboardResponse.text();

  assert(dashboardHtml.includes("Jornada da Equipe de Produção"), "Painel da produção não exibiu o heading esperado.");
  assert(dashboardHtml.includes(ownEventTitle), `Painel da produção não exibiu o evento vinculado ${ownEventTitle}.`);
  assert(!dashboardHtml.includes(foreignEventTitle), "Painel da produção vazou evento fora do vínculo institucional.");
  assert(dashboardHtml.includes("Agenda operacional filtrada pelas instituições vinculadas à sua atuação atual."), "Painel não sinalizou o recorte institucional esperado.");
...
async function submitCreate(jar, actionName, ownEventId, payload) {
  ...
  form.set(actionName, "");
  form.set("origin", "equipe-producao");
  ...
  const response = await request(jar, "/app/equipe-producao", {
    method: "POST",
    body: form,
  });
...
async function main() {
  ...
  const createPayload = {
    titulo: `Checkpoint smoke produção ${suffix}`,
    resumo: `Resumo operacional smoke ${suffix}`,
    status: "em_andamento",
    proximoPasso: `Fechar responsáveis e materiais da frente ${suffix}.`,
    apoioNecessario: `Confirmar escala e apoio de comunicação ${suffix}.`,
  };

  const updatePayload = {
    titulo: `Checkpoint smoke produção ${suffix}`,
    resumo: `Resumo operacional atualizado smoke ${suffix}`,
    status: "concluido",
  ...
  try {
    ...
    assert(createActionName, "Action do formulário de criação não encontrada na jornada de produção.");
    ...
    assert(createdTracking?.id, "Registro estruturado da produção não foi criado no banco.");
    ...
    const createdPageResponse = await request(jar, "/app/equipe-producao?tracking=created");
    assert(createdPageResponse.ok, `Página da produção após criação retornou HTTP ${createdPageResponse.status}.`);
    const createdPageHtml = await createdPageResponse.text();
    assert(createdPageHtml.includes("Checkpoint operacional salvo na jornada."), "Página não confirmou a criação do checkpoint operacional.");
    ...
    assert(updateActionName, "Action do formulário de atualização não encontrada na jornada de produção.");
    ...
    assert(updatedTracking?.id === createdTrackingId, "Registro atualizado da produção não foi localizado após o update.");
    assert(updatedTracking.resumo === updatePayload.resumo, "Resumo do checkpoint não foi atualizado no banco.");
    assert(updatedTracking.status === updatePayload.status, "Status do checkpoint não foi atualizado para concluído.");

    const updatedPageResponse = await request(jar, "/app/equipe-producao?tracking=updated");
    assert(updatedPageResponse.ok, `Página da produção após update retornou HTTP ${updatedPageResponse.status}.`);
    const updatedPageHtml = await updatedPageResponse.text();
    assert(updatedPageHtml.includes("Checkpoint recente atualizado nesta jornada."), "Página não confirmou a atualização do checkpoint operacional.");
    assert(updatedPageHtml.includes(updatePayload.resumo), "Painel não exibiu o resumo atualizado do checkpoint recente.");

    console.log(`Equipe produção scope smoke OK em ${baseUrl}`);
    ...
    console.log("- criação do checkpoint estruturado persistiu no banco com referência ao evento esperado");
    console.log("- atualização do checkpoint recente persistiu status concluído e resumo atualizado");
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.error(`Falha na prova runtime da equipe de produção: ${message}`);
